...
`src/roco_push_console/shared/provider_manifest.json` 是推送通道元数据的唯一来源。它包含：
...
10. 如果 Python/Worker 行为必须一致，更新 `tests/fixtures/cross_runtime_cases.json` 和 `cf-workers/test/cross-runtime.test.ts`。
...
Worker 版的源码在 `cf-workers/src/`，`cf-workers/_worker.js` 是生成产物。
...
控制台静态资源位于 `src/roco_push_console/static/`，模板位于 `templates/`。
...
- `cf-workers/test/worker.test.ts`：Worker 入口、配置、触发鉴权和端到端流程。
- `cf-workers/test/push-modules.test.ts`：Worker 推送模块边界、sender、HTTP 和脱敏。
- `cf-workers/test/cross-runtime.test.ts`：Python/Worker 共享行为 fixture。
